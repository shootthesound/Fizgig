...
**The full studio, as of 4.2.** H3 trains, previews and pauses/resumes like the other families — and all five workbench tools now work on H3 LoRAs too, with previews rendered as short clips judged by their middle frame. It was those tools, on real LoRAs, that produced the block map behind Optimised Likeness Learning below. **Context LoRA** works on H3 too (5.2): pick any existing H3 LoRA — AI-Toolkit files load as-is — and it rides frozen under the one you're training in every training step — and switches OFF for previews, so they show the LoRA as it deploys. That's the training-adapter workflow (Ostris's H3 assistant LoRA loads as-is) and it works just as well for a style or character LoRA you'll pair at inference. The output records which file it was trained against. LoRA runs only — not with fine-tuning.
