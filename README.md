...
Everything works on Krea 2: all five workbench tools, **Pause/Resume**, **Context LoRA**, **Adaptive LR**, reference images, the live sample override — **Weight averaging (EMA)** on by default at 0.98 (checkpoints and previews come from a running average of the adapter's recent steps; measured on both families, it lifts late-epoch likeness and steadies the epochs), and **LoKR training** (pick it from Network Type, factor 8 or below; you may need to lower the learning rate to 1e-4). Which to pick depends on the dataset: with smaller datasets, standard LoRA seems to achieve the higher quality, and it is ~20% faster, so it stays the default; for larger datasets LoKR is likely superior, but on smaller ones it can overfit before convergence is in a good place. Output is ComfyUI-ready.
