...
Two render regimes. **Dial** is the loop you turn sliders in: 4 steps with the bundled Turbo LoRA at full strength, at a smaller **Dial size** (⅔ of your chosen size by default — about twice as fast, ~4 s a move on a 5090). **Confirm** is the render you judge before saving: 6 steps at 75%, full size, the same regime as training previews. **Show early** puts a rough picture up after the second pass while the rest finishes. Everything you see is an exact render — nothing is approximated. A change to a late block skips the untouched blocks on the first pass (bit-for-bit the same result, up to a fifth faster).
