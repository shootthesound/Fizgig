...
**The text encoder stays parked in RAM.** The first prompt of a session streams the 32B encoder in (a couple of minutes, and the status line says so). On a machine with the RAM to hold it (about 24 GB free), it then stays parked in system RAM for the session, so every prompt or reference after that costs seconds instead of a fresh load, and editing a long prompt word by word is finally painless. It's released again if your RAM runs short, or when the studio unloads.
