...
fewer epochs — and the occasional deformed preview of full-model photo runs is gone. Since 10 Sep
the backward pass also stops at the window and the text token refiner's LoRA stays frozen on
those steps: about a quarter off every training step, and a same-seed A/B came out sharper, with
the epoch-to-epoch judder halved and likeness ahead at every epoch. Untick it
...
- **Optimised Likeness Learning** (default On) — photo and clip steps train the identity blocks (20-49) only, voice steps the audio zone (34-49). The backward stops at the window on all three step types and the text token refiner's LoRA is frozen on them, so the LoRA is genuinely a 20-49 LoRA (34-49 for voice): about 25% faster per step, and measured sharper and steadier epoch to epoch (`--likeness_full_backward` on the CLI restores the old full backward for an A/B). The measured best recipe for character and voice work, and confining clips this way trains video just as well while making clip steps far lighter on VRAM. Untick for style or scene training, which trains the whole model.
- **Train the text token refiner** (default Off, in Other Options) — the refiner is the model's bridge from the text encoder into the DiT and sets how every prompt is read, on every step. A LoRA on it moved that reading a little each epoch, which showed up as judder between previews and softer output; with it off, photos, voice and video all came out sharper and steadier, and a voice had its likeness by epoch 4 instead of 12-14. Leaving it off does not remove the model's ability to absorb a trigger word: the trigger is learned in the blocks' attention, where text meets image and sound. Files are eight keys lighter. LoRA runs only.
