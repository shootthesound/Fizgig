...
> ### 📰 Latest news
> - **Fizgig 5.3 — LoRA surgery on video.** Open a MiniMax H3 LoRA and see what every one of its 52 blocks does to a moving clip — the motion, the face, the sound — one click each, every one an exact render. Every preview is a 22-frame clip with its sound, played side by side in the app; Steps, Turbo and Render size boxes for a sub-4-second slider loop; a **block library** that renders every block switched off in the background so the ● beside each slider shows that block's effect instantly; **first / last frame** pinning, **Reference mode** (ref2va, `<Picture 1>` in the prompt), a **No-LoRA** pane, load-strength dials, short clips. **24 GB cards** run the int8 base in the studio with blocks streamed, and the text encoder parks in RAM on a 64 GB box. Also: HQQ moves to group 8, a third less base error at the same size (**[@rintic-13](https://github.com/rintic-13)**). [Details ↓](#repair-studio-on-h3-watch-the-clip-not-a-frame) · [Release notes](docs/RELEASE_NOTES_v5.3.0.md)
...
**Int8 attention.** A tick on the Clip row runs attention through comfy-kitchen's INT8 kernel (NVIDIA's, Apache-2.0, the one ComfyUI uses under `--use-ck-attention`) instead of PyTorch's. Attention grows with the square of the token count, so it is nothing on the slider loop and the biggest single cost on long or large clips: 3× faster per call on a 22-frame clip, 6–7× on 56 frames and the 1024 canvases (a 1024 × 1024, 56-frame render on a 5090 drops from about 83 s to about 58 s). Off by default — it is not an exact render (int8 Q, K and V). NVIDIA cards only; AMD and older cards fall back to PyTorch attention and the status line names which one rendered. The library keeps int8 and exact renders apart.
