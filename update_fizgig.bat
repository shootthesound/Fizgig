...
REM so a rewritten .bat is never read mid-run. call exit /b passes the update's exit code on.
cd /d "%~dp0"
if not exist "venv\Scripts\python.exe" (git checkout -- run_fizgig.bat 2>nul & git pull & echo. & echo WARNING: venv not found - run install_fizgig.bat to set it up. & pause & exit /b 1)
"venv\Scripts\python.exe" "update_fizgig.py" & pause & call exit /b %%errorlevel%%
